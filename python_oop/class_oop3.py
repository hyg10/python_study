...
# 상속 (inheritance)
# 부모 클래스가 기능을 물려받아, 새로운 클래스를 만드는 것
# 코드 중복 줄이고, 재사용성을 높이는 것이 목적임

# 다양성(Polymorphism)
# 파이썬에서는 다양성은 같은 메소드 이름인데 객체 타입에 따라 다른 동작을 하는 것을 말함
# 자바로 비교하면 부모 메소드 오버라이딩해서 동작 바인딩되는 개념임
# 일반적인 객체 지향의 다양성(부모 타입으로 후손 객체 여러 개를 다루는 기술)과는 다름
