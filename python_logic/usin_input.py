...
# # print(num + 100) # SyntaxError: cannot assign to True
# # 숫자와 문자는 계산할 수 없음

# # 입력된 숫자형문자를 숫자형으로 변환하고자 한다면 형변환함수 
# # 정수로 변환 : int('정수형문자')
# # 실수로 변환 : float('실수형문자')
# inum = int(num)
...
# 사용1 : 출력 내용을, 로 구분해서 나열
print('first : ', first, ', second :', second)

# 사용 2 : f'str'(fomat string) 사용 => 변수를 적용하려면 : f'출력문장 {변수명}'
print(f'더하기 결과 : {first} + {second} = {first + second}')

# 사용 3 : fomat() 함수 이용 => 주의 : 순서와 갯수가 반드시 일치해야 함
print('빼기 결과 : {} = {} = {}'.format(first, second, first - second))

# 사용 4 : fomat() 함수와 순번(index)을 이용 => 실수형 값의 소숫점 아래 자릿수도 지정할 수 있음 {:.2f}
