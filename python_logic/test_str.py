...
# 파이썬에서 문자열(str)은 시퀀스(sequence : 순차 나열자료형)로 취급됨
# 순차자료형은 나열된 값의 순번(인덱스, index)이 매겨짐 0부터 시작됨
# 동일한 시쿼스 자료형인 리스트(list), 튜플(tuple), 배열(array), 시리즈(Series) 들도 동일하게 적용됨
...
# 파이썬에서도 기록된 문자열값은 변경할 수 없음 (immutalbe)
# tt[1] = 'b' #TypeError: 'str' object does not support item assignment 값 못 바꿈

tt = 'banana'
print(tt)
print(id(tt))

print(tt.upper()) #tt가 참조하고 문자열을 cpu가 읽어가서 대문자로 변환하고, 메모리에 새로 기록함
print(id(tt.upper()))
print(tt)
...
print(tt.lower()) #tt가 참조하고 문자열을 cpu가 읽어가서 대문자로 변환하고, 메모리에 새로 기록함
print(id(tt.lower()))
print(tt)
...
print('|', tt4.strip(), '|', sep='') # 문자열 앞 뒤 공백을 제거함
...
# 없는 문자 조회하면 에러남
# print(tt5.index('k')) #ValueError: substring not found 없는 문자, 문자 조회
